...
function GetWeekday($date, $lang = 'vi')
{
    ...
    return $weekday . ', ' . date('d/m/Y', $date);
}
            ...
            if( $menu_item->menu_item_parent == 0 ) {
                ...
                foreach( $menu_items as $submenu ) {
                    if( $submenu->menu_item_parent == $parent ) {
                        ...
                        $menu_array[] = '<li class="nav-item"><a href="' . $submenu->url . '" class="'.$class.'">' . $submenu->title . '</a></li>' ."\n";
                    }
                }
                if( $bool == true && count($menu_array ) > 0 ) {
                    $class = $current_url == $menu_item->url ? 'nav-link active': 'nav-link';
                    $menu_list .= '<li class="nav-item">' ."\n";
                    $menu_list .= '<a class="'.$class.'" href="' . $menu_item->url . '">' . $menu_item->title . '</a>' ."\n";

                    $menu_list .= '<ul class="sidebar-sub-headers">' ."\n";
                    $menu_list .= implode( "\n", $menu_array );
                    $menu_list .= '</ul>' ."\n";
                } else {
                    $class = $current_url == $menu_item->url ? 'nav-link active': 'nav-link';
